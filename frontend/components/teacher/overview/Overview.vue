      ...
      >
        <template v-slot:table-row="props">
          <span v-if="props.column.field === 'action'">
            <RouterLink
              :to="`/teacher/student/${props.formattedRow['id']}`"
              class="btn btn-dark"
              >{{ $t('content.showStudent') }}</RouterLink
            >
          </span>
          <span v-else>{{ props.formattedRow[props.column.field] }}</span>
        </template>
      </vue-good-table>
      <button class="btn btn-dark csv" @click="exportTableToCSV">CSV</button>
    </div>
  </div>
</template>
  ...
  methods: {
    async fetchData() {
      ...
      for (let i = 0; i < response.length; i++) {
        ...
        const pocetOdovzdanych = response2.filter(
          (item) => item.submitted === 1
        ).length;
        const pocetBodov = response2.reduce(
